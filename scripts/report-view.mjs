...
function projectComparisonText(pc) {
  ...
  return `Project comparison: ${pc.labels.current} vs ${pc.labels.baseline}; requests ${number(requests.current)} vs ${number(requests.baseline)} (delta ${delta(requests.delta)}); estimated cost ${costOf(cost.current)} vs ${costOf(cost.baseline)} (delta ${costOf(cost.delta)})`;
}
