...
serialized access, MCP exposure, and recovery when Codex reports that a stored session
no longer exists. ShareView is the first configured client: its Claude Code `/rex`
command reaches the wrapper through the narrow `ask_rex` MCP tool.
